...
/**
 * Copy of vanilla StructureBlock
 */
class ExtendedStructureBlock(settings: Settings?) : BlockWithEntity(settings), OperatorBlock {


    companion object {
        val MODE: EnumProperty<StructureBlockMode> = STRUCTURE_BLOCK_MODE
        val CODEC: MapCodec<ExtendedStructureBlock> =
            createCodec { settings: Settings? -> ExtendedStructureBlock(settings) }
    }
    ...
    override fun createBlockEntity(pos: BlockPos, state: BlockState?): BlockEntity {
        return ExtendedStructureBlockEntity(pos, state!!)
    }
    ...
    override fun onUse(
        ...
        if (blockEntity is ExtendedStructureBlockEntity && state != null) {
            if (!world.isClient) {
